	...
	<aside id="sc-sidebar-main" class="sc-sidebar-info-fixed">
		<div id="sc-sidebar-main-offcanvas-bar" v-touch:swipe.left="closeSidebar" class="uk-offcanvas-bar">
			<div class="sc-sidebar-main-scrollable" data-sc-scrollbar="visible-y">
				<ScMenuList :menu-data="menuAccount"  :is-parent="true"></ScMenuList>
...
export default {
	...
	methods: {
		 async getCategory(){
			 const menuAccountData = AES.encrypt(JSON.stringify(menuAccount),this.secrectKey);
             localStorage.setItem('menuAccountData', menuAccountData.toString());
        },
		getmenuData(){
			const decrypted2 = AES.decrypt(localStorage.getItem('menuAccountData'), this.secrectKey);
		    const decryptedObject = decrypted2.toString(enc.Utf8);
		    var dataq = JSON.parse(decryptedObject)
			this.menuAccount = dataq;				
			},
